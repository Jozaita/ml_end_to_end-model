...
import torch
from ml_end_to_end.models.common.io_utils import cache_gcs_resource_locally, copy_file
from ml_end_to_end.models.common.utils import get_local_rank,get_global_rank, global_rank_zero_first, local_rank_zero_first
from ml_end_to_end.utils.utils import get_logger
from omegaconf import DictConfig, OmegaConf

MODELS_MODULE_PATH = "ml_end_to_end/models"
TEMP_MODELS_MODULE_PATH = "temp_modul/models"
MODELS_CONFIG_PATH = "model_config.yaml"
STATE_DICT_PATH = "model_state_dict.pth"
EXPORTED_MODEL_FILE_NAME = "exported_model.tar.gz"


class TarModelExporter():
    def __init__(self, model_state_dict_path: str, model_config: Any, tar_model_export_path: str = None)->None:
        ...
        with  global_rank_zero_first():
            if get_global_rank() in [0,-1]:
                ...
                with tarfile.open(local_tar_path,"w:gz") as tar:
                    ...
                    tar.add(model_config_path, arcname=MODELS_CONFIG_PATH)
    ...
    def save_model_config(self) -> str:
        model_config_save_path = os.path.join(tempfile.gettempdir(),MODELS_CONFIG_PATH)
...
class TarModelLoader:
    ...
    def load(self)->Any:
        temp_export_path = "/tmp/temp_ml_end_to_end"
    ...
    def extract_tar_gz(self,export_path:str) -> None:
        ...
        with tarfile.open(local_exported_model_path, "r:gz") as tar:
            tar.extractall(export_path)

    def load_model_config(self, model_dir: str) -> Any:
        model_config = OmegaConf.load(f"{model_dir}/{MODELS_CONFIG_PATH}")
    ...
    def _replace_module_in_model_config(self,config:Any)->Any:
        for key,value in config.items():
            if isinstance(value, (dict,DictConfig)):
                config[key] = self._replace_module_in_model_config(value)
            if key == "_target_":
