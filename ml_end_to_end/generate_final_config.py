...
import mlflow
from ml_end_to_end.utils.config_utils import get_config, get_config_and_dict_config, save_config_as_yaml
from ml_end_to_end.utils.mlflow_utils import activate_mlflow, log_artifacts_for_reproducibility, log_training_hparams
...
def generate_final_config(config:"Config",dict_config:DictConfig):
    with activate_mlflow(config.infrastructure.mlflow.experiment_name,
        ...
        config_save_dir = Path("./ml_end_to_end/configs/automatically_generated")
        config_save_dir.mkdir(parents=True,exist_ok=True)
        (config_save_dir /"__init__.py").touch(exist_ok=True) 
        #(config_save_dir /".gitignore").touch(exist_ok=True) 
