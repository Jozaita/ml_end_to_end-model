...
class HuggingFaceTokenizationTransformationConfig(TransformationConfig):
    _target_: str = 'ml_end_to_end.models.transformations.HuggingFaceTokenizationTransformation'
...
class CustomHuggingFaceTokenizationTransformationConfig(HuggingFaceTokenizationTransformationConfig):
    pretrained_tokenizer_name_or_path: str = "gs://ml_end_to_end/data/processed/rebalanced_split/trained_tokenizer"
    max_sequence_length: int = 200


def setup_config() -> None:
    cs = ConfigStore.instance()
    cs.store(name="text_classification_data_module_schema", group="tasks/data_module/transformation", node=HuggingFaceTokenizationTransformationConfig)
