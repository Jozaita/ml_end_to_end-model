...
class MetricComparerConfig:
    _target_:str = "ml_end_to_end.evaluation.model_selector.MetricComparer"
...
class ModelSelectorConfig:
    _target_:str = "ml_end_to_end.evaluation.model_selector.ModelSelector"
    mlflow_run_id: Optional[str] = SI("${infrastructure.mlflow.run_id}")
    must_be_better_metric_comparers: Optional[dict[str, MetricComparerConfig]] = field(default_factory=lambda:{})
    to_be_thresholded_metric_comparers: Optional[dict[str, MetricComparerConfig]] = field(default_factory=lambda:{})
    threshold:float = 0.0

@dataclass
class CyberBullyingModelSelectorConfig(ModelSelectorConfig):
    must_be_better_metric_comparers: Optional[dict[str, MetricComparerConfig]] = field(default_factory=lambda:{
