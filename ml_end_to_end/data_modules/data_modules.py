...
from ml_end_to_end.data_modules.datasets import TextClassificationDataset
from ml_end_to_end.models.transformations import HuggingFaceTokenizationTransformation, Transformation
from torch.utils.data import Sampler,BatchSampler,Dataset,DataLoader,default_collate
...
class TextClassificationDataModule(DataModule):
    def __init__(self,
        ...
        def tokenization_collate_fn(batch: list[tuple[str,int]])->tuple[BatchEncoding,Tensor]:
            text,labels = default_collate(batch)
    ...
    def setup(self,stage:Optional[str]=None)->None:
        if stage == 'fit' or stage is None:
            self.train_dataset = TextClassificationDataset(self.train_path, self.text_column_name,self.label_column_name)
            self.dev_dataset = TextClassificationDataset(self.dev_path, self.text_column_name,self.label_column_name)
        if stage == 'test':
            self.test_dataset = TextClassificationDataset(self.test_path, self.text_column_name,self.label_column_name)
